...
const els={
  q:$('q'), list:$('list'), editor:$('editor'), formTitle:$('formTitle'),
...
function setStatus(t,cls){ els.syncStatus.textContent=t; els.syncStatus.className='chip '+(cls||'') }
...
async function sha256(text){
  ...
  // fallback (not strong, but avoids total block on very old Safari)
  return btoa(text||'');
}
function hasLocalPassword(){ return !!localStorage.getItem(PASS_KEY) }
async function tryUnlock(pass){ const saved=localStorage.getItem(PASS_KEY); if(!saved) return true; return saved===await sha256(pass) }
async function setLocalPassword(pass){ if(!pass){localStorage.removeItem(PASS_KEY);return} localStorage.setItem(PASS_KEY, await sha256(pass)) }
...
function getStore(){ try{ return JSON.parse(localStorage.getItem(STORE_KEY)) || initStore() }catch{ return initStore() } }
...
async function uploadJsonNew(folderId, name, obj){
  const meta={ name, parents:[folderId] }, data=JSON.stringify(obj);
  ...
  const r=await gapi.client.request({ path:'/upload/drive/v3/files', method:'POST', params:{uploadType:'multipart'},
    headers:{'Content-Type':'multipart/related; boundary="'+boundary+'"'}, body }); return r.result;
}
async function updateJson(fileId, obj){
  const meta={ name:DRIVE_FILE }, data=JSON.stringify(obj);
  const {body,boundary}=multipart(meta,data,'application/json');
...
async function ensureRoots(a, store){
  await ensureSignedIn(a);
  const folderId = store.meta.drive?.folderId || await findOrCreateFolder(DRIVE_FOLDER);
  const fileId   = store.meta.drive?.fileId   || (await uploadJsonNew(folderId, DRIVE_FILE, store)).id;
  const imagesId = store.meta.drive?.imagesFolderId || await findOrCreateFolder(IMAGES_FOLDER, folderId);
  store.meta.drive={ folderId, fileId, imagesFolderId:imagesId }; return store.meta.drive;
}
async function pullMerge(store){
  try{
...
function recipeThumb(r){ const first=(r.images?.[0]?.webViewLink)||''; const url=first || (r.photo||''); return url?`<img alt="" src="${attr(url)}" style="width:100%;height:160px;object-fit:cover;border:1px solid var(--stroke);border-radius:12px">`:'' }
function renderList(){
...
window.delRecipe=async(id)=>{ const s=getStore(); const r=s.recipes.find(x=>x.id===id); if(!r) return;
  ...
  s.recipes=s.recipes.filter(x=>x.id!==id); setStore(s); renderList(); pushNow('delete');
}
function saveRecipe(){
...
function init(){
  ...
  // Lock controls
  $('unlockBtn').onclick=async()=>{ const ok=await tryUnlock($('lockInput').value); if(ok){ showLock(false); $('lockInput').value=''; $('lockMsg').textContent=''; } else $('lockMsg').textContent='Wrong password.'; };
  $('setPassBtn').onclick=async()=>{ const val=$('lockInput').value.trim(); if(!val){ $('lockMsg').textContent='Enter a new password first.'; return } await setLocalPassword(val); $('lockMsg').textContent='Password set. Use Unlock.'; };
}
init();
