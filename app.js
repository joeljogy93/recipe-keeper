/* Recipe Keeper (per-recipe files) */

// ---- Constants / helpers
const PASS_KEY = 'rk_local_pass_plain_v1';           // password (local-only)
...
const els = {
  ...
  setPassBtn: $('setPassBtn'), lockMsg: $('lockMsg'),
...
const fields = {
  title: $('title'), category: $('category'), prep: $('prep'),
...
// ---- Theme (auto default; toggle saves choice)
(function initTheme(){
...
async function connectAndLoad(){
  try{
    ...
    m.rootId=rootId; m.recipesId=recipesId; m.imagesId=imagesId; setMeta(m);
...
function setStatus(t){ els.syncStatus.textContent=t }
...
function renderList(data){
  ...
  const catActive = window._catSelected || 'All';
  const filtered = (data||[]).filter(r=>{
    const catOk = (catActive==='All' || r.category===catActive);
    const text = [r.title||'',r.ingredients||'',r.steps||'',r.notes||''].join('\n').toLowerCase();
    return catOk && (!q || text.includes(q));
  });

  if(!filtered.length){ els.list.innerHTML='<div class="card muted">No recipes yet. Add one.</div>'; return }
...
function renderCatsUI(){
  ...
  els.catBar.innerHTML = cats.map(c=>`<span class="pill ${c===window._catSelected?'active':''}" onclick="selectCat('${attr(c)}')">${esc(c)}</span>`).join('');
...
function updatePreview(){
  ...
  $('preview').innerHTML = `<h3 style="margin:6px 0 2px">${esc(title)}</h3>
...
window.deleteRecipe = async (fileId)=>{
  if(!confirm('Delete this recipe file from Drive?')) return;
...
async function saveRecipe(){
  ...
  if(!recipe.title) recipe.title='(untitled)';

  if(currentFileId){
    await updateJsonFile(currentFileId, recipe);
  }else{
    const name = recipe.title.replace(/[^\w\- ]+/g,'').slice(0,60) + ' ' + uid() + '.json';
    currentFileId = await createJsonFile(m.recipesId, name, recipe);
  }
  clearForm(); closeEditor(); await refreshList();
  window._editingImages = [];
}

// ---- Image upload (adds to editing set)
els.uploadImgBtn.onclick = ()=>{
  const m=getMeta(); if(!m.imagesId) return alert('Connect Google Drive first.');
  els.imgInput.click();
};
els.imgInput.addEventListener('change', async e=>{
  ...
  try{
    ...
    setTimeout(()=>els.uploadStatus.textContent='',1200);
    e.target.value='';
  }
});

// ---- Category manager
function usageCount(recipes){ const u={}; getCats().forEach(c=>u[c]=0); (recipes||[]).forEach(r=>{ if(r.category&&u[r.category]!=null) u[r.category]++ }); return u }
async function buildUsage(){ // simple usage scan via list (titles only)
  const files = await listRecipeFiles(); const items=[];
  for(const f of files){ const r=await loadRecipe(f.id); if(r){ items.push(r) } }
  return usageCount(items);
}
...
window.renameCat = async (i)=>{
  const cats=getCats(); const u=await buildUsage(); if((u[cats[i]]||0)>0) return alert('In use');
  const name=prompt('New name:',cats[i]); if(!name) return; if(cats.includes(name)) return alert('Already exists');
  cats[i]=name.trim(); setCats(cats); refreshCatListDom(await buildUsage());
};
window.deleteCat = async (i)=>{
  const cats=getCats(); const u=await buildUsage(); if((u[cats[i]]||0)>0) return alert('In use');
  if(!confirm('Delete category?')) return; cats.splice(i,1); setCats(cats); refreshCatListDom(await buildUsage());
};
...
function wire(){
  ...
  $('unlockBtn').onclick=()=>{
    ...
    if(tryUnlock(pass)){ showLock(false); els.lockMsg.textContent=''; els.lockInput.value=''; }
    else els.lockMsg.textContent='Wrong password.';
  };
  $('setPassBtn').onclick=()=>{ els.settingsDlg.showModal() };
  $('savePassBtn').onclick=()=>{ const p=els.newPass.value.trim(); if(!p) return els.settingsMsg.textContent='Password cannot be empty.'; setPass(p); els.settingsMsg.textContent='Password saved (this device only).'; els.newPass.value=''; els.lockMsg.textContent='Password set. Use Unlock.'; };
  ...
  $('signOutBtn').onclick=async()=>{ try{ const a=gapi.auth2.getAuthInstance(); if(a) await a.signOut() }catch(_){} els.driveSetupCard.classList.remove('hidden'); setStatus('Offline') };
  ...
  // Categories
  $('manageCatsBtn').onclick=async()=>{ refreshCatListDom(await buildUsage()); els.catDlg.showModal() };
